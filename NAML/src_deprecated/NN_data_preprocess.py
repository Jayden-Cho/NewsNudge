...
def parse_news(source, target, category2int_path, word2int_path, mode):
    ...
    def parse_row(row):
        new_row = [
            row.id,
            category2int[str(row.category)] if row.category in category2int else 0,
...
if __name__ == '__main__':
    ...
    print('Parse news')
    parse_news(path.join(data_dir, 'NN_news.tsv'),
               path.join(data_dir, 'NN_news_parsed.tsv'),
               path.join(data_dir, 'NN_category2int.tsv'),
               path.join(data_dir, 'NN_word2int.tsv'),
               mode='test')
